const currentYear = new Date().getFullYear();

const translations = {
  en: {
    home: "Home",
    services: "Services",
    about: "About",
    contact: "Contact",
    getStarted: "Get Started",
    ourServices: "Our Services",
    everythingYouNeed: "Everything you need to succeed",
    digitalStrategy: "Digital Strategy",
    support: "24/7 Support",
    globalReach: "Global Reach",
    aboutUs: "About us",
    aboutDescription: "We are a creative web design company specializing in crafting custom, user-friendly websites. From responsive design and e-commerce solutions to SEO and graphic design, we help your business stand out online and deliver an exceptional user experience.",
    links: "Links",
    contactUs: "Contact Us",
    contactDescription: "Get in touch with our team to learn more about how we can help you succeed.",
    name: "Name",
    email: "Email",
    message: "Message",
    sendMessage: "Send Message",
    quickLinks: "Quick Links",
    contactInfo: "Contact Info",
    phone: "+1 (555) 123-4567",
    address: "123 Business St, Suite 100",
    footer: `© ${currentYear} OS Design v1 — All rights reserved. Powered by GitHub. Styled with Tailwind CSS. Created by Tiger3homs.`,
    heroTitlePart1: "Transform Your",
    heroTitlePart2: "Digital Presence",
    heroDescription: "We help businesses grow by creating exceptional digital experiences that connect with their audience.",
    features: [
      {
        title: "Digital Strategy",
        description: "Build a roadmap for your digital success with our comprehensive strategy services."
      },
      {
        title: "24/7 Support",
        description: "Round-the-clock support to ensure your business never misses a beat."
      },
      {
        title: "Global Reach",
        description: "Connect with customers worldwide through our international network."
      }
    ]
  },
  sv: {
    home: "Hem",
    services: "Tjänster",
    about: "Om oss",
    contact: "Kontakt",
    getStarted: "Kom igång",
    ourServices: "Våra tjänster",
    everythingYouNeed: "Allt du behöver för att lyckas",
    digitalStrategy: "Digital strategi",
    support: "24/7 Support",
    globalReach: "Global räckvidd",
    aboutUs: "Om oss",
    aboutDescription: "Vi är ett kreativt webbdesignföretag som specialiserar sig på att skapa anpassade, användarvänliga webbplatser. Från responsiv design och e-handelslösningar till SEO och grafisk design hjälper vi ditt företag att sticka ut online och leverera en exceptionell användarupplevelse.",
    links: "Länkar",
    contactUs: "Kontakta oss",
    contactDescription: "Kontakta vårt team för att lära dig mer om hur vi kan hjälpa dig att lyckas.",
    name: "Namn",
    email: "E-post",
    message: "Meddelande",
    sendMessage: "Skicka meddelande",
    quickLinks: "Snabblänkar",
    contactInfo: "Kontaktinformation",
    phone: "+1 (555) 123-4567",
    address: "123 Business St, Suite 100",
    footer: `© ${currentYear} OS Design v1 — Alla rättigheter förbehållna. Drivs av GitHub. Stajlad med Tailwind CSS. Skapad av Tiger3homs.`,
    heroTitlePart1: "Förvandla din",
    heroTitlePart2: "Digitala närvaro",
    heroDescription: "Vi hjälper företag att växa genom att skapa exceptionella digitala upplevelser som ansluter till deras publik.",
    features: [
      {
        title: "Digital strategi",
        description: "Bygg en färdplan för din digitala framgång med våra omfattande strategitjänster."
      },
      {
        title: "24/7 Support",
        description: "Support dygnet runt för att säkerställa att ditt företag aldrig missar ett slag."
      },
      {
        title: "Global räckvidd",
        description: "Anslut med kunder över hela världen genom vårt internationella nätverk."
      }
    ]
  },
  ar: {
    home: "الرئيسية",
    services: "الخدمات",
    about: "معلومات عنا",
    contact: "اتصل بنا",
    getStarted: "ابدأ الآن",
    ourServices: "خدماتنا",
    everythingYouNeed: "كل ما تحتاجه للنجاح",
    digitalStrategy: "استراتيجية رقمية",
    support: "دعم 24/7",
    globalReach: "الوصول العالمي",
    aboutUs: "معلومات عنا",
    aboutDescription: "نحن شركة تصميم ويب إبداعية متخصصة في إنشاء مواقع ويب مخصصة وسهلة الاستخدام. من التصميم المتجاوب وحلول التجارة الإلكترونية إلى تحسين محركات البحث والتصميم الجرافيكي، نساعد عملك على التميز عبر الإنترنت وتقديم تجربة مستخدم استثنائية.",
    links: "روابط",
    contactUs: "اتصل بنا",
    contactDescription: "تواصل مع فريقنا لمعرفة المزيد حول كيفية مساعدتك على النجاح.",
    name: "الاسم",
    email: "البريد الإلكتروني",
    message: "الرسالة",
    sendMessage: "إرسال الرسالة",
    quickLinks: "روابط سريعة",
    contactInfo: "معلومات الاتصال",
    phone: "+1 (555) 123-4567",
    address: "123 Business St, Suite 100",
    footer: `© ${currentYear} تصميم OS الإصدار 1 — جميع الحقوق محفوظة. مدعوم من GitHub. مُنسق باستخدام Tailwind CSS. تم الإنشاء بواسطة Tiger3homs.`,
    heroTitlePart1: "حول",
    heroTitlePart2: "وجودك الرقمي",
    heroDescription: "نساعد الشركات على النمو من خلال إنشاء تجارب رقمية استثنائية تتواصل مع جمهورها.",
    features: [
      {
        title: "استراتيجية رقمية",
        description: "قم ببناء خارطة طريق لنجاحك الرقمي من خلال خدماتنا الاستراتيجية الشاملة."
      },
      {
        title: "دعم 24/7",
        description: "دعم على مدار الساعة لضمان عدم تفويت عملك لأي نبضة."
      },
      {
        title: "الوصول العالمي",
        description: "تواصل مع العملاء في جميع أنحاء العالم من خلال شبكتنا الدولية."
      }
    ]
  }
};

export default translations;